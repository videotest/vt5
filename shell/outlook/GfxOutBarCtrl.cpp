...
IMPLEMENT_DYNCREATE(CGfxOutBarCtrl, CControlBar)
...
void CGfxOutBarCtrl::SetBarStyleEx( DWORD dwStyle )
{
	/*
...
BEGIN_MESSAGE_MAP(CGfxOutBarCtrl, CControlBar)
...
{
	dwFlags = dwFlag;
	return CControlBar::CreateEx(WS_EX_STATICEDGE, NULL, NULL, dwStyle|WS_CHILD, rect, pParentWnd, nID);
...
{
	if( GetFolderCount() == 0 || iSelFolder == -1 )
		return htNothing;
	...
	if (ht == htFolder)
	{
		...
		if (::GetCapture() == NULL)
		{
			...
			for (;;)
			{
				...
				if (CControlBar::GetCapture() != this) break;
...
{
	if( GetFolderCount() == 0 || index == -1 )
		return CSize( 0, 0 );

	ASSERT(iFolder >= 0 && iFolder < GetFolderCount());
	CBarFolder * pbf = (CBarFolder *) arFolder.GetAt(iFolder);

	if( index >= pbf->GetItemCount() )
		return CSize( 0, 0 );
...
//!!!end
	...
	if (IsSmallIconView())
	{
		if (ima)
		{
			...
			if (!bOnlyImage)
			{
				...
				pDC->TextOut(rc.left, rc.top+4, CString(pi->cItem));
			}
	...
	{
//!!!andy
		CRect rcHilight = rcItem;
		//GetIconRect(iSelFolder, iLastSel, rcHilight);
		//rcHilight.InflateRect(1,1);
		rcHilight.InflateRect( -1, 1 );
		pDC->Draw3dRect(rcHilight, crDkShadowBorder, cr3dFace);
//end
	}
...
//		GetIconRect(iSelFolder, iLastItemHighlighted, rc);
		GetItemRect(iSelFolder, iLastItemHighlighted, rc);
//!!!end
		rc.InflateRect(1,1);
...
/*	iHitInternal1 = HitTestEx(point, iHitInternal2);
	...
	CString	_ID_GFX_SMALLICON, _ID_GFX_LARGEICON, _ID_GFX_REMOVEITEM, _ID_GFX_RENAMEITEM;
	_ID_GFX_SMALLICON.LoadString( ID_GFX_SMALLICON );::LanguageLoadCString
	_ID_GFX_LARGEICON.LoadString( ID_GFX_LARGEICON );::LanguageLoadCString
	_ID_GFX_REMOVEITEM.LoadString( ID_GFX_REMOVEITEM );::LanguageLoadCString
	_ID_GFX_RENAMEITEM.LoadString( ID_GFX_RENAMEITEM );::LanguageLoadCString
...
void CGfxOutBarCtrl::AnimateFolderScroll(const int iFrom, const int iTo)
{
	...
	if (iTo > iFrom)
	{
		...
		if (pWnd1) 
		{
			...
			pWnd1->ScreenToClient( &rect1 );

						
			CPoint	ptDelta = rect2.TopLeft()-rect1.TopLeft();



			CPoint ovpt = pDC->SetViewportOrg(+ptDelta.y, frc.Height()+ptDelta.y);
		...
		if (pWnd2) 
		{
			CRect	rect1;
			CRect	rect2;
			pWnd2->GetWindowRect( &rect1 );
			pWnd2->GetClientRect( &rect2 );
			pWnd2->ScreenToClient( &rect1 );

						
			CPoint	ptDelta = rect2.TopLeft()-rect1.TopLeft();

			CPoint ovpt = pDC->SetViewportOrg(+ptDelta.x, frc.Height()+ptDelta.y);
	...
	else
	{
		...
		if (pWnd2) 
		{
			...
			CPoint ovpt = pDC->SetViewportOrg(ptDelta.x, frc.Height()*2+ptDelta.y);
			BOOL bPrev = pWnd2->ShowWindow(SW_SHOW);
			pWnd2->SendMessage(WM_ERASEBKGND, (WPARAM) pDC->GetSafeHdc(), (LPARAM) 0);
			pWnd2->SendMessage(WM_PAINT, (WPARAM) pDC->GetSafeHdc(), (LPARAM) 0);
			if (!bPrev) pWnd2->ShowWindow(SW_HIDE);
...
void CGfxOutBarCtrl::OnUpdateCmdUI(CFrameWnd* pTarget, BOOL bDisableIfNoHndler)
{
}

CSize CGfxOutBarCtrl::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{		  
	CSize	sizeDefault( GetValueInt( GetAppUnknown(), "General", "OutlookBarWidth", 100 ), 500 );
	return sizeDefault;
}
