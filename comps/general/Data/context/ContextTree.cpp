...
CContextCtrl::CContextCtrl()
{
    ...
    m_nETimerID			= 0;          // For drag-selection
	m_nETimerCount		= 0;
...
{
	return hItem && m_list.Find(hItem);
}
...
HCONTEXTITEM CContextCtrl::InsertItem(LPCTSTR lpszItem, HCONTEXTITEM hParent, HCONTEXTITEM hInsertAfter, int nHeight )
{
	CVINSERTSTRUCT insert;
	insert.hInsertAfter = hInsertAfter;
	insert.hParent		= hParent;
...
//	SetItemNumber(hItem, false);
	...
	POSITION visPos = m_listVisible.Find(hItem);
	if (visPos)
		m_listVisible.RemoveAt(visPos);
...
//	if( !IsWindowVisible() )
		return;
    if (nIDEvent != WM_VSCROLL && nIDEvent != WM_MOUSEMOVE && nIDEvent != WM_LBUTTONDOWN)
 		return;

	if (nIDEvent == WM_LBUTTONDOWN)
	{
		m_nETimerCount++;
		if (m_nETimerCount > 10)
		{
			m_nETimerCount = 0;
			KillTimer(m_nETimerID);
			m_nETimerID = 0;
		}
		return;
	}
...
//	CancelToolTips();
	...
	// Set WM_LBUTTONDOWN timer

	CWnd * pFocus = CWnd::GetFocus();
	if (pFocus != this)
	{
		if (pFocus && pFocus->GetParent() == this && hItem == GetItemFocus())
		{
			m_MouseMode = MOUSE_NOTHING;
			m_bMouseButtonDown   = false;
			m_LeftClickDownPoint = point;
	/*
			if (!m_nETimerID && hItem && hItem == GetItemFocus())
			{
				m_nETimerCount = 0;
				m_nETimerID = SetTimer(WM_LBUTTONDOWN, m_nTimerInterval, 0);
			}
	*/
			m_LeftClickDownItem = 0;//GetItemFocus();
			SetFocus();
			return;
		}
		SetFocus();
...
//	TRACE("lButtonDown : %s\n", m_LeftClickDownItem->GetText());
	...
	if (!ItemIsHeader(m_LeftClickDownItem))
	{
		...
		{
			// check WM_LBUTTONDOWN timer
			if (IsItemEditable(m_LeftClickDownItem))
			{	
				if (m_nETimerID)
				{
					// kill WM_LBUTTONDOWN timer
					m_nETimerCount = 0;
					KillTimer(m_nETimerID);
					m_nETimerID = 0;
...
//	TRACE("lButtonUp : %s\n", hHitItem->GetText());
	...
	if (!m_bMouseLDblClick)
	{
		...
		if (!ItemIsHeader(hHitItem))
		{
			...
			if ((uFlag & CVHT_ONITEMLABEL) == CVHT_ONITEMLABEL && 
...
{
	TRACE("CContextCtrl::OnRButtonUp\n");
    m_TitleTip.Hide();  // hide any titletips
...
{
	TRACE("CContextCtrl::OnRButtonDown\n");
...
bool CContextCtrl::DrawIcon(CDC * pDC, CRect ImageRect, HCONTEXTITEM hItem)
{
	...
	HWND hOwner = ::GetParent(*this);
	if (IsWindow(hOwner)){
		VERIFY(::SendMessage(hOwner,WM_DRAWITEM, 0, LPARAM(&dis)));
	}
