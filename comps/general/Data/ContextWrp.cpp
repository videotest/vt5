...
long GetTypeOrderDefault(LPCTSTR lpstrType)
{
	if (strcmp(lpstrType, szArgumentTypeImage)==0)
		return 0;
	else if (strcmp(lpstrType, szDrawingObjectList)==0)
		return 1;
	else if (strcmp(lpstrType, szArgumentTypeBinaryImage)==0)
		return 2;
	else if (strcmp(lpstrType, szArgumentTypeObjectList)==0)
		return 3;
	else if (strcmp(lpstrType, "StatTable")==0)
		return 4;
	else if (strcmp(lpstrType, "StatObject")==0)
		return 5;
	return 100;
}
...
{
	long lDefault = GetTypeOrderDefault(lpstrType);
	long lTypeOrder = GetValueInt(GetAppUnknown(), "\\ContextViewer\\TypeOrder", lpstrType, lDefault);
	return lTypeOrder;
}
...
//	CTimeTest tt (true, "CContextWrpObject::UpdateSelections");
	...
	// get types count
	long nCount = 0;
	sptrC->GetObjectTypeCount(&nCount);
	...
	for (long i = 0; i < nCount; i++)
	{
		...
		while (lPos)
		{
			// get object 
			IUnknownPtr sptrObj;
			if (FAILED(sptrC->GetNextSelected(bstrType, &lPos, &sptrObj)) || sptrObj == 0)
				continue;
...
//			ASSERT(nCount == nNumber);
			...
			if (hItem)
				m_Tree.SetItemNumber(hItem, nSelCount++);
...
HCONTEXTITEM CContextWrpObject::AddBaseObject(GuidKey Key, bool bSort)
{
	...
	CString strType = ::GetObjectKind(sptrBase);


	// create item data
	CContextItemData * pdata = new CContextItemData(strName, Key, lNum, strType);
	if (!pdata)
		return 0;

	// insert item
	HCONTEXTITEM hItem = m_Tree.InsertItem(pdata->GetDisplayName(), CVI_ROOT);
	if (!hItem)
		return 0;
...
*/
	long nCount = 0;
	m_sptrContext->GetObjectTypeCount(&nCount);
...
bool CContextWrpObject::AddObject(HCONTEXTITEM hParent, IUnknown * punkObject, bool bSort)
{
	...
	if (!(hItem = m_Tree.InsertItem(pdata->GetDisplayName(), hParent, CVI_LAST )))
		return false;
...
void CContextWrpType::OnSetFocus(CWnd* pOldWnd) 
{
	...
	IUnknownPtr sptrObj = GetFocusObject(false);
	if (sptrObj == 0)
	{
		IUnknownPtr sptrUnkView = m_sptrView != 0 ? m_sptrView : GetActiveContext(false);
...
void CContextWrpType::OnActivateItem(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	_variant_t vart_arg[2] = { bstrType, (IUnknown*)sptrObj };
	IScriptSitePtr	sptr_script_site = ::GetAppUnknown();
	HRESULT hr = sptr_script_site->Invoke( _bstr_t("DataContext_BeforeActivateItem"), vart_arg, 2, NULL, fwFormScript | fwAppScript );
...
//	if we have no active item now, we need to activate
	...
	_variant_t vart_arg[3] = { nCount, bstrType, (IUnknown*)sptrObj };
	_variant_t varRes;
	IScriptSitePtr	sptr_script_site = ::GetAppUnknown();
	HRESULT hr = sptr_script_site->Invoke( _bstr_t("DataContext_BeforeNumberItem"), vart_arg, 3, &varRes, fwFormScript | fwAppScript );
	if (SUCCEEDED(hr) && (varRes.vt == VT_I4 && varRes.lVal == 0 || varRes.vt == VT_I2 && varRes.iVal == 0))
		return;
...
//	_variant_t vart_arg[2] = { nCount, bstrType };
//	IScriptSitePtr	sptr_script_site = ::GetAppUnknown();
//	sptr_script_site->Invoke( _bstr_t("DataContext_OnNumberItem", vart_arg, 2, 0, fwFormScript | fwAppScript );
...
void CContextWrpType::OnEndLabelEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	CContextItemData * pData = (CContextItemData *)m_Tree.GetItemData(hItem);
	CString strOldName = pData ? ::GetObjectName(pData->GetPointer(false))/*GetName()*/ : strNewName;
...
//	BOOL bPrevRedraw = SetRedraw(false);
	...
	if (!strNewName.IsEmpty())
	{
		...
		CString strObjectName = ::GetObjectName(sptrObj);
		*pResult = strObjectName == strNewName;

		if (strObjectName != strNewName)
		{
			HCONTEXTITEM hItem = FindObjectItem(sptrObj);
			if (hItem)
			{
				CContextItemData * pData = (CContextItemData *)m_Tree.GetItemData(hItem);
				if (pData)
					pData->SetName(strObjectName);
				m_Tree.SetItemText(hItem, strObjectName);
			}
		}
...
void CContextWrpType::OnBeginLabelEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	if( ::IsDBaseRootObject( pData->GetPointer(false), 0 ) )
		return;
...
int CALLBACK CContextWrpType::CompareTypeFunc(HCONTEXTITEM hItem1, HCONTEXTITEM hItem2, LPARAM lParamSort)
{
	...
	if (pData1 && pData2)
	{
		int r = pData1->GetTypeOrder()-pData2->GetTypeOrder();
		if (r != 0)
			return r;
	}
...
{
	//CString strType(GetTypeAliase( szType) );
...
HCONTEXTITEM CContextWrpType::GetTypeItem(LPCTSTR szType)
{
	...
	CString strType( szType );
	HCONTEXTITEM hItem = 0;
	if (!m_mapTypes.Lookup(strType, (void*&)hItem))
		return 0;
...
//			rect.OffsetRect(1, 1);
	...
	/*
	IUnknown* punkData = 0;
	punkData = GetActiveDocument();
	if( !punkData )
		return;

	IDBaseDocumentPtr ptrDBaseDoc( punkData );
	...
	ptrDBaseDoc->IsDBaseObject( pData->GetPointer(false), &bDBaseObject, &guidRootObject );
	if( bDBaseObject )
	{
		CBitmap bmpDBaseObject;
		bmpDBaseObject.LoadBitmap( IDB_DBASE_OBJECT );		

		BITMAP bm;
		::ZeroMemory( &bm, sizeof( bm ) );

		VERIFY(bmpDBaseObject.GetBitmap(&bm));		
   
		CSize sizeBmp( bm.bmWidth, bm.bmHeight );		

		CDC memdc;
		memdc.CreateCompatibleDC( pDC );
		CBitmap* pOldBitmap = memdc.SelectObject( &bmpDBaseObject );

		int x, y;
		x = rect.right - sizeBmp.cx + 2;
		y = rect.bottom - sizeBmp.cy+ 2;
		
		
		
		pDC->BitBlt(x, y, sizeBmp.cx, sizeBmp.cy, &memdc, 0, 0, SRCCOPY); // copy
...
//		SetFocusObject(sptrObj);
		...
		if (hItem)
		{
			...
			if (pFocusWnd != &m_Tree)
				m_Tree.SendMessage(WM_KILLFOCUS, WPARAM(pFocusWnd ? pFocusWnd->m_hWnd : 0));
		}
	}
...
//	CTimeTest tt (true, "CContextWrpType::SetNumeric()");
	CWnd * pFocusWnd = CWnd::GetFocus();
	if (pFocusWnd != &m_Tree && pFocusWnd != this)
		SetFocus();

	HCONTEXTITEM hFocusItem = (CWnd::GetFocus() == &m_Tree) ? m_Tree.GetItemFocus() : m_Tree.GetPrevItemFocus();
	if (!hFocusItem)
		return;
...
IUnknown * CContextWrpType::GetFocusObject(bool bAddRef)
{
	...
	if (!hFocusItem || !m_Tree.IsValid(hFocusItem))
		return 0;
