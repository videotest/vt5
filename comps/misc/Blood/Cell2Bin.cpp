...
_ainfo_base::arg CCell2BinInfo::s_pargs[] = 
{       
	{"Img",       szArgumentTypeImage, 0, true, true },
	{"Cell2Bin",    szArgumentTypeObjectList, 0, false, true },
...
void CCell2Bin::Smoothing( BYTE *p_arg, BYTE *p_res , int cx , int cy, int Mask )
{
	if(Mask<=1) 
	{
...
void CCell2Bin::DilateColor(BYTE **sel, int cx, int cy, int mask, byte cFore, byte cForeNew, byte cBack)
...
void CCell2Bin::ExpandDarkArea(color **src, byte **dst, CSize size, int tolerance)
{
...
void CCell2Bin::SelectByRadius(color **src, byte **dst, CSize size, int tolerance, RECT *dstRect)
{
	int cBack=int(colorSumBack/nBackCount);
	int cFore=int(colorSum/(nInserted-nInserted0));
	...
	dstRect->left=0;
	dstRect->top=0;
	dstRect->right=size.cx;
	dstRect->bottom=size.cy;
}

bool CCell2Bin::SelectCell(color **src, byte **dst, int x0, int y0, CSize size, RECT *dstRect)
{
	dstRect->left=x0;
	dstRect->top=y0;
	dstRect->right=x0;
...
void CCell2Bin::DeleteSmallSegments(byte **dst, CSize size)
{
...
void CCell2Bin::CalcHist(color **src, int cx, int cy, int *hist)
...
void CCell2Bin::CalcBack2(int *hist, color &back1, color &back2, color &threshold)
{
...
void CCell2Bin::Calibrate(color **src, byte **bin, int cx, int cy)
{
	//--------- Калибровка ---------
	const int NC=256; 
