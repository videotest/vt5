...
void CStatObjectView::default_init()
{
	m_nSplitterType = 0;
...
//////////////////////////////////////////////////////////////////////
IUnknown* CStatObjectView::DoGetInterface( const IID &iid )
{
...
//////////////////////////////////////////////////////////////////////
void CStatObjectView::DoAttachObjects()
{
	...
	while( lpos_selected )
	{
		...
		if( punk_object )
		{
			...
			INamedDataPtr sptrND = punk_object;
			if( sptrND == 0 )
			{
	...
	if( m_list_attached.count() )
	{
		for (TPOS lpos_lst = m_list_attached.head(); lpos_lst; lpos_lst = m_list_attached.next(lpos_lst))
		{
			...
			{
				load_from_ndata( ::GetAppUnknown() );
				store_to_ndata( ptr_object );
				CopyObjectNamedData( ::GetAppUnknown(), ptr_object, SECT_STATUI_STAT_TABLE_USERDATA , 1 );

				{

					m_wndChart.SetClassFile( (char*)::GetValueString( ptr_object, SECT_STATUI_ROOT, CLASS_FILE, 0 ) );
					m_wndLegend.SetClassFile( m_wndChart.GetClassFile() );
					m_wndTable.SetClassFile( m_wndChart.GetClassFile() );
				}
			}
			else
			{
...
void CStatObjectView::attach_data()
{
	for (TPOS lpos_lst = m_list_attached.head(); lpos_lst; lpos_lst = m_list_attached.next(lpos_lst))
	{
		...
		bool bError = _check_for_error( ptr_object );
		m_bError = bError;
	...
	if( m_bError || m_list_attached.count() == 0 || m_nSplitterType < 0 || m_nSplitterType > 3 )
	{ 
...
//////////////////////////////////////////////////////////////////////
LRESULT	CStatObjectView::DoMessage( UINT nMsg, WPARAM wParam, LPARAM lParam )
{
	LRESULT	lResult = 0;
	switch( nMsg )
	{
...
//////////////////////////////////////////////////////////////////////
long CStatObjectView::on_create( CREATESTRUCT *pcs )
{
/*
...
//////////////////////////////////////////////////////////////////////
long CStatObjectView::on_destroy()
{
	if(IsWindow())
		UnsubclassWindow();
	for (TPOS lpos_lst = m_list_attached.head(); lpos_lst; lpos_lst = m_list_attached.next(lpos_lst))
...
//GoodForPaint:
	...
	if( m_bError || 0==m_list_attached.count())
	{
...
//////////////////////////////////////////////////////////////////////
long CStatObjectView::on_erase_background( HDC hDC )
{
	return 1L;
}

//////////////////////////////////////////////////////////////////////
long CStatObjectView::on_size( int cx, int cy )
{
...
//////////////////////////////////////////////////////////////////////
long CStatObjectView::on_context_menu( HWND hWnd, int x, int y )
{
	if( !m_bShowContext )
		return 0;
...
//////////////////////////////////////////////////////////////////////
HRESULT CStatObjectView::GetMatchType( BSTR bstrObjectType, DWORD *pdwMatch )
{
	_bstr_t	bstr( bstrObjectType );
	if( !strcmp( bstr, szTypeStatObject ) )
		*pdwMatch = mvFull;
	else
		*pdwMatch = mvNone;
...
//////////////////////////////////////////////////////////////////////
#include <mmsystem.h>
void CStatObjectView::OnNotify( const char *pszEvent, IUnknown *punkHit, IUnknown *punkFrom, void *pdata, long cbSize )
{
		...
		!strcmp( pszEvent, szEventChangeObject ) /*&& m_bPrintMode*/ ||  !strcmp( pszEvent, szEventActiveContextChange ) )
	{
		...
		if(m_list_attached.count())
		{
			...
			IStatObjectPtr sptr_stat_object = m_list_attached.get( m_list_attached.head() );
			CStatisticObject* rso=(CStatisticObject*)((IStatObject*)sptr_stat_object);
			if(CStatisticObject::Calculated==rso->m_Warning
				&& !m_bPrintMode)
			{
		_clear_attached();
		DoAttachObjects();
				//if(!strcmp( pszEvent, szEventNewSettingsView ) )
				//{
				//	if(!rso->RestoreByClassifier(rso->m_vecClasses))
				//	{
				//		rso->LoadColorsNames();
				//	}
				//}
...
//Print support
HRESULT CStatObjectView::GetPrintWidth( int nMaxWidth, int *pnReturnWidth, BOOL *pbContinue, int nUserPosX, int *pnNewUserPosX )
{
	if( !(m_dwViews & vtTableView) )
	{
...
//////////////////////////////////////////////////////////////////////
HRESULT CStatObjectView::GetPrintHeight( int nMaxHeight, int *pnReturnHeight, BOOL *pbContinue, int nUserPosY, int *pnNewUserPosY )
{
	if( !(m_dwViews & vtTableView) )
	{
		IScrollZoomSitePtr ptr_zoom = Unknown();
...
//////////////////////////////////////////////////////////////////////
HRESULT CStatObjectView::Print( HDC hdc, RECT rectTarget, int nUserPosX, int nUserPosY, int nUserPosDX, int nUserPosDY, BOOL bUseScrollZoom, DWORD dwFlags )
{
	if( m_bError )
	{
...
//Persist Impl
//////////////////////////////////////////////////////////////////////
HRESULT CStatObjectView::GetClassID(CLSID *pClassID )
{
...
HRESULT CStatObjectView::ShowView( DWORD dwView )
{
	//if( m_dwViews == dwView )
	//	return S_FALSE;
...
HRESULT CStatObjectView::GetViewVisibility( DWORD *pdwView )
{
	if( !pdwView )
		return S_FALSE;
...
const SIZE CStatObjectView::_recalc_layout(SIZE sizeRect)
{
	if( m_list_attached.count() )
	{
		BOOL bLock = 0;
		...
		for (TPOS lpos_lst = m_list_attached.head(); lpos_lst; lpos_lst = m_list_attached.next(lpos_lst))
		{
			...
			if( sptrS )
				sptrS->GetValidateLock( &bLock );

			if( bLock )
				break;
		}

		m_wndChart.m_bShowErrorMsg = false; //!bLock;
	}

	SIZE sizeNew = {0};

	if( /*!m_bShowen ||*/ m_bLockRecalc || m_list_attached.count() == 0 )
		return sizeNew;

	if( m_list_attached.count() == 0 || _check_for_error() )
	{
		if( m_bError )
...
void CStatObjectView::load_from_ndata( INamedDataPtr sptrND )
{
	if( sptrND == 0 )
		return;
	m_bLockRecalc = false;
...
void CStatObjectView::store_to_ndata( INamedDataPtr sptrND )
{
	if( sptrND == 0 )
		return;
...
const RECT CStatObjectView::_calc_min_view_size()
{
	RECT rcPane0_min, rcPane1_min, rcPane2_min;
	DWORD vtPane0 = 0, vtPane1 = 0, vtPane2 = 0;
...
bool CStatObjectView::_is_ndata_empty( IUnknownPtr sptr )
{
...
bool CStatObjectView::_check_for_error( IUnknownPtr sptrData )
{
...
void CStatObjectView::_clear_attached()
{
	m_bReadyToShow=false;
	m_list_attached.clear();
	m_wndChart.clear_attached();
...
#pragma intrinsic(_bittest)
	...
	struct splitter{
		...
		HWND SetSplitter(CStatObjectView* pStatView,
...
HWND CStatObjectView::SetSplitterStruct(void)
{
