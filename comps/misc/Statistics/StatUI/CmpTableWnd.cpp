...
namespace ViewSpace 
{
	...
	CCmpTableWnd::CCmpTableWnd() // : m_grid(1), m_gridTotal(2), m_gridClass(0)
		: m_rowLastVisible(-1)
		, m_bPrinting(false)
		, _rStatCmpView(*(CCmpStatObjectView*)((char*)this-offsetof(CCmpStatObjectView,m_wndTable)))
	{
		m_bPrintMode = false;
	...
	CCmpTableWnd::~CCmpTableWnd()
	{
		deinit();
	}

	void CCmpTableWnd::deinit()
	{
		if( m_pColumnInfo )
	...
	void CCmpTableWnd::add_attach_data(IUnknownPtr sptrData ) 
	{ 
		m_sptrAttached = _rStatCmpView.m_pCmpStatObject;
	}

	LRESULT CCmpTableWnd::on_paint()
	{
	...
	void CCmpTableWnd::DoDraw( HDC hDC, RECT rcPaint, double fZoom, bool bPrint  )
	{
		//		fZoom =1./fZoom;
	...
	RECT CCmpTableWnd::get_column_rect( int iIndex )
	{
		RECT rc = {0};
	...
	RECT CCmpTableWnd::get_row_rect( int iIndex )
	{
		RECT rc = {0}, rc2 = {0};
		{
			HWND hwnd_header = ListView_GetHeader( m_Grid );
	...
	const CRect& CCmpTableWnd::get_min_rect()
	{
		return 	m_rcMinRect;
	}

	void CCmpTableWnd::load_from_ndata(  INamedDataPtr sptrND )
	{
		deinit();
		...
		if( lKeySz + m_lCustomDataCount )
			m_pColumnInfo = new XColumnInfo[lKeySz + m_lCustomDataCount + 2];

		if( lKeySz + m_lCustomDataCount )
		{
			{
				CComVariant vColWidth;
				HRESULT hr=sptrND->GetValue(bstrDOC_COL_WIDTH,&vColWidth);
				...
				{
					m_pColumnInfo[0].nWidth = vColWidth.lVal;
				}
			}
			{
				...
				if(VT_EMPTY==vClassColWidth.vt)
				{
					char szColumn[15] = {0};
					sprintf( szColumn, "%s%d", COLUMN_COLOR_ID, 0 );
					m_pColumnInfo[1].nWidth = ::GetValueInt( sptrND, SECT_STATUI_STAT_TABLE_WIDTH, szColumn , m_pColumnInfo[1].nWidth );
				}
				else
				{
					m_pColumnInfo[1].nWidth = vClassColWidth.lVal;
				}
			}
		}
		...
		for( int i = 0; i < lKeySz + m_lCustomDataCount; i++ )
		{
			...
			sprintf( szColumn, "%s%d", COLUMN_COLOR_ID, i + 2 );

			m_pColumnInfo[i + 2].lIndex = i;

			long lKey = 0;

			if( i < lKeySz ) 
				m_pColumnInfo[i + 2].lParamKey = lKey = plKey_arr[i];
			...
			// Get color width from NamedData
			{
				...
				HRESULT hr=sptrND->GetValue(bstrSTAT_PARAMS+_bstr_t(_variant_t(m_pColumnInfo[i + 2].lParamKey))+"\\Color", &vColColor);
				int n=0;
				if(VT_BSTR==vColColor.vt)
				{
					int r=0,g=0,b=0;
					n=swscanf(vColColor.bstrVal, L"(%d,%d,%d)",&r,&g,&b);
					m_pColumnInfo[i + 2].clColumnColor = RGB(r,g,b);
				}
				if(n<3)
					m_pColumnInfo[i + 2].clColumnColor = ::GetValueColor( sptrND, SECT_STATUI_STAT_TABLE_COLOR, szColumn , m_pColumnInfo[i + 2].clColumnColor );
			}
			...
			// Get column width from NamedData
			{
				...
				HRESULT hr=sptrND->GetValue(bstrSTAT_PARAMS+_bstr_t(_variant_t(m_pColumnInfo[i + 2].lParamKey))+L"\\Width", &vColWidth);
				if(VT_EMPTY==vColWidth.vt)
					m_pColumnInfo[i + 2].nWidth = ::GetValueInt( sptrND, SECT_STATUI_STAT_TABLE_WIDTH, szColumn , m_pColumnInfo[i + 2].nWidth );
				else
				{
					m_pColumnInfo[i + 2].nWidth = vColWidth.lVal;
				}
			}

			if( i < lKeySz ) 
			{
				strcpy( m_pColumnInfo[i + 2].szName, (char*)::GetValueString( sptrND, szParamSect, PARAM_NAME , 0 ) );
				strcpy( m_pColumnInfo[i + 2].szFormat, (char*)::GetValueString( sptrND, szParamSect, FORMAT_STRING , 0 ) );
				if(0==strlen( m_pColumnInfo[i + 2].szFormat))
					strcpy(m_pColumnInfo[i + 2].szFormat,"%g");
				m_pColumnInfo[i + 2].lOrder = _get_param_order_by_key( sptrND, lKey, 0 );
			}
			else
			{
				_bstr_t name;
				...
				m_pColumnInfo[i + 2].lUsedID = i - lKeySz;

				if( lPos )
					strcpy( m_pColumnInfo[i + 2].szName, m_mapColumnName.get( lPos ) );
				else
					strcpy( m_pColumnInfo[i + 2].szName, "" );

				TPOS lPos2 = m_mapColumnOrder.find(i - lKeySz);

				if( lPos2 )
					m_pColumnInfo[i + 2].lOrder = m_mapColumnOrder.get( lPos2 );
				else
					m_pColumnInfo[i + 2].lOrder = 0;

				m_pColumnInfo[i + 2].bUserData = true;
	...
	void CCmpTableWnd::store_to_ndata( INamedDataPtr sptrND )
	{
		if( sptrND == 0 )
			return;
			...
			return;
		{
			...
			HRESULT hr=sptrND->SetValue(bstrDOC_COL_WIDTH, _variant_t(m_pColumnInfo[0].nWidth));
			hr=sptrND->SetValue(bstrCLASS_COL_WIDTH, _variant_t(m_pColumnInfo[1].nWidth));

			for( int i = 0; i < m_lParamCount + m_lCustomDataCount; i++ )
			{
				_bstr_t bstrFullKey=bstrSTAT_PARAMS+_bstr_t(_variant_t(m_pColumnInfo[i + 2].lParamKey));
				SetValueColor(sptrND,bstrFullKey,"Color",_variant_t(m_pColumnInfo[i + 2].clColumnColor));
	...
	bool CCmpTableWnd::create( DWORD style, const RECT &rect, const _char *pszTitle, HWND parent, HMENU hmenu, const _char *pszClass )
	{
	...
	void CCmpTableWnd::_resize_child( RECT rect )
	{
		m_rcFull = rect;
...
//			::ShowWindow( m_wHotBtn.handle(), SW_SHOW );
	...
	LRESULT CCmpTableWnd::on_size(short cx, short cy, ulong fSizeType)
	{
		RECT rc;
		::GetClientRect( m_hwnd, &rc );
		if(IsRectEmpty(&rc))
...
//		_ASSERTE(m_Grid.GetColumnCount()==0);
		...
		{
			IStatObjectDispPtr sptrStatObject = statObjects[0].second._pStat;
...

		{
			...
			for( int i = 2; i < m_lParamCount + m_lCustomDataCount + 2; i++ )
				plArrOrder[i-2] = m_pColumnInfo[i].lOrder;

			sort_templ2( m_pColumnInfo + 2, plArrOrder, m_lParamCount + m_lCustomDataCount , 1 );
...

		{
			...
			col.iOrder=0;
			col.iSubItem=0;
			CString sDoc; sDoc.LoadString(IDS_DOCUMENT);
			col.pszText=(LPTSTR)(LPCTSTR)sDoc;
			m_Grid.InsertColumn(0, &col);

			col.cx=m_pColumnInfo[1].nWidth;
			col.iOrder=1;
			col.iSubItem=1;
			col.pszText=(LPTSTR)(LPCTSTR)m_strClassNameCol;
			m_Grid.InsertColumn(1, &col);
		}

		int nCols=m_lParamCount + m_lCustomDataCount+2;
		int sizeStats=statObjects.size();
		int nRows=(lClassSz+1)*sizeStats;
		...
		for( int i = 0; i < lClassSz; i++ )
		{
			...
			int iRow=i*sizeStats;
			m_strVals[iRow][1]=_rStatCmpView.m_pCmpStatObject->get_class_name( lClassNum );
			for(int iStat=0;iStat<sizeStats;++iStat)
			{
				int iiRow = iRow + iStat;
				CLVItem& info=m_arrClassItem[iiRow];
				info.SetBkColor(RGB(234,230,224));
				info.SetForeColor(statObjects[iStat].second._clrCmpStat);
				CString fileName=statObjects[iStat].first;
				fileName=fileName.Right(fileName.GetLength()-fileName.ReverseFind('\\')-1);
				m_strVals[iiRow][0]=fileName;
				LRESULT b=m_Grid.InsertItem(iRow,LPSTR_TEXTCALLBACK);
				DWORD dw;
				if(b<0)
					dw=GetLastError();

				LVITEM lvi={LVIF_TEXT};
				lvi.pszText=(0==iStat)?LPSTR_TEXTCALLBACK:0;
				lvi.iItem=iiRow;
				lvi.iSubItem=1;
				m_Grid.SetItem(&lvi);	   
			}
		...
		for( int i = 0; i < lClassSz; i++ )
		{
			...
			for(int iStat=0,iRow=i*sizeStats; iStat<sizeStats; ++iStat,++iRow)
			{
				IStatObjectDispPtr sptrStatObject = statObjects[iStat].second._pStat;
				for( int j = 0; j < m_lParamCount + m_lCustomDataCount; j++ )
				{
					long lKey = 0;

					if( !m_pColumnInfo[j + 2].bUserData )
						lKey =  m_pColumnInfo[j + 2].lParamKey;
					...
					{
						if( !m_pColumnInfo[j + 2].bUserData )
							szValue.Format(m_pColumnInfo[j + 2].szFormat, fValue );
						else
							szValue=_get_user_value( m_pColumnInfo[j + 2].lUsedID, iRow);
					}
					else
					{
						if( hr == S_FALSE )
							szValue.LoadString( App::handle(), IDS_NOVAL);
						else
							szValue.LoadString( App::handle(), IDS_NOEXPR);
					}

					m_strVals[iRow][j+2]=szValue;
					LVITEM lvi={LVIF_TEXT};
		...
		// insert parameter columns
		IStatObjectPtr sptrSt = statObjects[0].second._pStat;
		for( int i = 0; i < m_lParamCount + m_lCustomDataCount; i++ )
		{
			long lKey = 0;

			if( !m_pColumnInfo[i + 2].bUserData )
				lKey = m_pColumnInfo[i + 2].lParamKey;
			...
			strcpy( name, m_pColumnInfo[i + 2].szName );
			...
			LVCOLUMN col={LVCF_FMT|LVCF_WIDTH|LVCF_ORDER|LVCF_SUBITEM|LVCF_TEXT, LVCFMT_CENTER};
			col.cx=m_pColumnInfo[i+2].nWidth;
			col.iOrder=i+2;
			col.iSubItem=i+2;
			col.pszText=name;
			m_Grid.InsertColumn(i+2, &col);
		}

		// fill total rows
		int iRow=lClassSz*sizeStats;
		m_strVals[iRow][1]=m_strAllName;
		for(int iStat=0;iStat<sizeStats;++iStat,++iRow)
		{
			IStatObjectDispPtr sptrStatObject = statObjects[iStat].second._pStat;
			CLVItem& info=m_arrClassItem[iRow];
			info.SetBkColor(RGB(234,230,224));
			info.SetForeColor(statObjects[iStat].second._clrCmpStat);
			CString fileName=statObjects[iStat].first;
			fileName=fileName.Right(fileName.GetLength()-fileName.ReverseFind('\\')-1);
			m_strVals[iRow][0]=fileName;
			LRESULT b=m_Grid.InsertItem(iRow,LPSTR_TEXTCALLBACK);
			...
			for( int i = 0; i < m_lParamCount + m_lCustomDataCount; i++ )
			{
				...
				{
					if( !m_pColumnInfo[i + 2].bUserData )
						szValue.Format(m_pColumnInfo[i + 2].szFormat, fValue );
					else
						szValue=_get_user_value( m_pColumnInfo[i + 2].lUsedID, -1);
				}
				else
				{
					if( hr == S_FALSE )
				...
				m_strVals[iRow][i+2]=szValue;
				LVITEM lvi={LVIF_TEXT};
				lvi.pszText=(LPTSTR)(LPCTSTR)m_strVals[iRow][i+2];
				lvi.iItem=iRow;
				lvi.iSubItem=i+2;
				m_Grid.SetItem(&lvi);	   
			}
...
//			_ASSERTE(nCols==m_Grid.GetColumnCount());
	...
	void CCmpTableWnd::calc_min_rect( bool bPrintMode )
	{
		if( !m_bMinSizeCalc )
		{
			RECT rc = { 0, 0, 200, 200 };
	...
	CString CCmpTableWnd::_get_user_value( long lCol, long lRow )
	{
	...
	LRESULT CCmpTableWnd::on_hscroll(unsigned scrollCode, unsigned pos, HWND hwndScroll)
	{
	...
	LRESULT CCmpTableWnd::on_vscroll(unsigned code, unsigned pos, HWND hwndScroll)
	{
	...
	LRESULT CCmpTableWnd::on_notify(uint idc, NMHDR *pnmhdr)
	{
		switch(pnmhdr->code)
		{
		default:
...
//							}else if(1==col){
				...
				if(!m_Grid.bBlockChanged){
					...
					if( (pNMHEADER->pitem->mask & HDI_WIDTH) == HDI_WIDTH )
					{
						...
						if(0<=iColumn && iColumn<nCols){
							...
							if(iWidth>0){
								HRESULT hr;
								if(INamedDataPtr pND=_rStatCmpView.m_pCmpStatObject->Unknown()){
									if(0==iColumn){
										hr=pND->SetValue(bstrDOC_COL_WIDTH, _variant_t(m_pColumnInfo[0].nWidth));
									}else if(1==iColumn){
	...
	LRESULT CCmpTableWnd::handle_message(UINT m, WPARAM w, LPARAM l)
	{
		switch( m )
		{
	...
	int CCmpTableWnd::GetTableWidth() const
	{
		int lVirtualWidth = 0;
	...
	int CCmpTableWnd::GetTableHeight() const
	{
	...
	POINT CCmpTableWnd::RowCol(const POINT& pt)
	{
		CRect rcHeader; Header_GetItemRect( m_Grid.GetHeader(), 0, &rcHeader );
		CRect rc; m_Grid.GetItemRect(0, &rc, LVIR_BOUNDS );
	...
	RECT CCmpTableWnd::RectHitTest(const POINT& pt)
			{
	...
	bool CCmpTableWnd::GetPrintWidth(int nMaxWidth, int& nReturnWidth, int nUserPosX, int& nNewUserPosX)
			{
...
//			nNewUserPosX = nUserPosX+nReturnWidth;
	...
	bool CCmpTableWnd::GetPrintHeight(int nMaxHeight, int& nReturnHeight, int nUserPosY, int& nNewUserPosY)
	{
		bool bContinue = false;
		int nRows=m_strVals.GetRowsCount();
		if(nRows<=0)
	...
	int CCmpTableWnd::Print(CDCHandle& dc, CRect rectTarget, CRect RectDraw)
	{
		if (!dc || !m_pColumnInfo)
			return 0;
		...
		dc.SetWindowExt(RectDraw.Width(), RectDraw.Height());
		...
		{
			if( m_bPrinting && 
				(0==_rStatCmpView.m_pCmpStatObject || 0==_rStatCmpView.m_pCmpStatObject->_statObjects.size()) )
				return 0;
