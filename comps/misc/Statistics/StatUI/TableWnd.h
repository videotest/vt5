...
namespace ViewSpace 
{
	...
	class CStatObjectView;

	class CTableWnd : public win_impl
	{
...
//		CString m_strTitle;
	...
	public:
		...
		{ 
			...
			m_sptrAttached = m_list_attached.get( m_list_attached.head() );
		};
		...
		void set_active_simple( int nindex ) 
		{
			...
			for (TPOS lp = m_list_attached.head(); lp; lp = m_list_attached.next(lp), lid++)
			{
				if( lid == nindex )
				{
					m_sptrAttached = m_list_attached.get( lp ); 
					break;
				}
			}
		}
