...
VARIANT CVT5ProfileManager::_GetProfileValue(LPCTSTR lpSection, LPCTSTR lpEntry, bool bIgnoreMethodic)
{
	...
	//sergey 01/02/06 è 11.05.06
	CString s=lpEntry;
	//if(s==_T("ColorCamera"))-11.05.06
	if(s==_T("ColorCamera")|| s==_T("CaptureFromPreview")|| s==_T("Fish"))
     sSecName += CString("\\")+SectionName(lpSection,true);	
	else
	sSecName += CString("\\")+SectionName(lpSection,bIgnoreMethodic);	
	//sSecName += CString("\\")+SectionName(lpSection,bIgnoreMethodic);	
	//end
	CString sKeyName(lpEntry);
	BSTR bstrSecName = sSecName.AllocSysString();
	BSTR bstrKeyName = sKeyName.AllocSysString();
	sptrND->SetupSection(bstrSecName);
	sptrND->GetValue(bstrKeyName, &var);
	::SysFreeString(bstrSecName);
	::SysFreeString(bstrKeyName);
	return var;
}

VARIANT CVT5ProfileManager::GetDefaultProfileValue(LPCTSTR lpSection, LPCTSTR lpEntry, bool bIgnoreMethodic)
{
	sptrINamedData sptrND(GetAppUnknown());
	CString sSecName(_T("Input"));
	IUnknown *punkCM = _GetOtherComponent( GetAppUnknown(), IID_IDriverManager );
...
//	if (lpSection)
	...
	sSecName += "-Default";
	CString sKeyName(lpEntry);
	sptrND->SetupSection(_bstr_t((LPCTSTR)sSecName));
	sptrND->GetValue(_bstr_t((LPCTSTR)sKeyName), &var);
	return var;
}
...
//	if (lpSection)
		sSecName += CString("\\")+SectionName(lpSection,bIgnoreMethodic);
	CString sKeyName(lpEntry);
	if (!sKeyName.IsEmpty() && !sSecName.IsEmpty())
	{
		sptrND->SetupSection(_bstr_t((LPCTSTR)sSecName));
		sptrND->SetValue(_bstr_t((LPCTSTR)sKeyName), var);
	}
}
...
int CVT5ProfileManager::GetDefaultProfileInt(LPCTSTR lpSection, LPCTSTR lpEntry, int nDef, bool bIgnoreMethodic)
{
	if (IsVT5Profile())
	{
...
double CVT5ProfileManager::GetDefaultProfileDouble(LPCTSTR lpSection, LPCTSTR lpEntry, double dDef, bool *pbDefValueUsed, bool bIgnoreMethodic)
{
	if (IsVT5Profile())
	{
		VARIANT var = GetDefaultProfileValue(lpSection, lpEntry);
...
CString CVT5ProfileManager::GetDefaultProfileString(LPCTSTR lpSection, LPCTSTR lpEntry, LPCTSTR lpDefault, bool bIgnoreMethodic)
{
	if (IsVT5Profile())
	{
		VARIANT var = GetDefaultProfileValue(lpSection, lpEntry);
...
void CVT5ProfileManager::WriteProfileString(LPCTSTR lpSection, LPCTSTR lpEntry, LPCTSTR lpValue, bool bIgnoreMethodic)
{
	if (IsVT5Profile())
	{
		...
		WriteProfileValue(lpSection, lpEntry, var, bIgnoreMethodic);
	}
	else
...
static void CopySection(CString sDriverName, CString sSec)
{
	...
	sSecDefault += "-Default";
	sptrND->SetupSection(bstr_t((LPCTSTR)sSecDefault));
	long nCount;
