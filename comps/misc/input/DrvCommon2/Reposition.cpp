...
void CRepositionManager::Reposition(CSize szBase, CRect &rcBase, CSize &szTotal)
{
	...
	{
		double dRatio = double(nCenterY)/double(nCenterX);
		if (nLeft+nCenterX+nRight>m_szMaxClient.cx)
		{
			nBaseX = nCenterX = m_szMaxClient.cx-nLeft-nRight;
			nBaseY = int(nCenterX*dRatio);
			nCenterY = max(nBaseY,nCenMinY);
		}
		if (nTop+nCenterY+nBottom>m_szMaxClient.cy)
		{
			nBaseY = nCenterY = m_szMaxClient.cy-nTop-nBottom;
			nBaseX = int(nCenterY/dRatio);
			nCenterX = max(nBaseX,nCenMinX);
		}
	...
	rcBase = CRect(nLeft,nTop,nLeft+nBaseX,nTop+nBaseY);
