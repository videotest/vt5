...
const CIntFuncInfo FInfo=
	{0, 0, 6, 0,"CCamGrabImage","CCamGrabImage",0};
...
CImageArg res1 =
{
	{sizeof(CImageArg),
	iptImage,
	"Result image of function CCamGrabImage",
	"Image on output"},
	NULL
};
...
class CCCamDrv : public CFGDrvBase
{
	CDriverCCam *m_pDrv;
public:
...
CDriverCCam *CCCamDrv::GetDriver()
{
	if (m_pDrv == NULL)
		m_pDrv = new CDriverCCam;
	return m_pDrv;
}
...
BOOL CCCamDrv::fgInit()
{
	return TRUE;
}

void CCCamDrv::fgExit()
{
//	DeinitHardware();
}

BOOL CCCamDrv::fgIsHdwPresent()
{
//	InitHardware();
	return TRUE;//m_bHdwOK;
}

UINT CCCamDrv::fgGetHdwFlags()
{
	return FG_HASSETUPDLG|FG_UICONTROLLABLE;
}

int CCCamDrv::fgGetFormatDescr(FGFormatDescr **fd)
{
	*fd = fds;
	return 1;
}

void CCCamDrv::fgSetFormat(int iNum)
{
}

BYTE *CCCamDrv::fgGetLine(short LineNum)
{
	return NULL;
}

HWND CCCamDrv::fgCreateSetupDialog(HWND hWndParent, BOOL bModal)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	CDriverCCam *pDrv = new CDriverCCam;
...
BOOL CCCamDrv::fgGetBrContrLimits( int *iBrMin, int *iBrMax, int *iCntrMin, int *iCntrMax )
{
	*iBrMin = *iCntrMin = 0;
	*iBrMax = *iCntrMax = 0;
	return TRUE;
...
BOOL CCCamDrv::fgGetBrContr( int *iBr, int *iContr )
{
	*iBr = 0;
	*iContr = 0;
	return FALSE;
...
BOOL CCCamDrv::fgSetBrContr( int iBr, int iContr, BOOL bAuto )
{
	return FALSE;
}

LPCSTR CCCamDrv::fgGetDeviceName()
{
	return "C-Cam";
}

int CCCamDrv::fgGetFormat()
{
	return 0;
}

void CCCamDrv::fgSetExposTime( int iTime )
{
}

int CCCamDrv::fgGetExposTime()
{
	return 0;
}

void CCCamDrv::fgForse8bit( BOOL bForce )
{
}

BOOL CCCamDrv::fgIsForsed8bit()
{
	return TRUE;
}

void CCCamDrv::fgSetUIType(int nType)
{
}

void CCCamDrv::fgSetXferType(int nType)
{
}

BOOL CCCamDrv::fgIsReverseBits()
{
	return FALSE;
}

void CCCamDrv::fgRelease()
{
	if (m_pDrv)
		m_pDrv->Release();
	m_pDrv = NULL;
...
const char *CCCamDrv::GetFunctionName()
{
	return "CCamGrabImage";
}


std_dll_c void CCamGrabImage( CInvokeStruct *pInvokeStruct, void **pArguments, void *pResult, void **pErrInfo )
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
	...
	CDriverCCam *pDrv = g_CCamDrv.GetDriver();
	if (!bSilent)
	{
//		_AFX_THREAD_STATE *pThreadState = AfxGetThreadState();
//		pThreadState->m_pRoutingFrame = NULL;
