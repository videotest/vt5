	...
	: CDialog(CPreviewDlg::IDD, pParent)
{
	...
	if (!m_pPreview && !m_pOverlay)
		AfxThrowNotSupportedException();
	m_bInited = false;
	m_bOvrMode = false;
	m_bOvrActive = false;
	m_bBoardOK = false;
	m_bUseOverlay = false;//m_pOverlay != NULL && g_BaumerProfile.GetProfileInt(_T("Settings"), _T("UseOverlay"), 0, true)?true:false;
	m_bUseStretch = false;//g_BaumerProfile.GetProfileInt(_T("Settings"), _T("UseStretch"), 1, true)?true:false;
//	m_pRedrawHook = NULL;
...
BOOL CPreviewDlg::OnInitDialog() 
{
	...
	// 1. First, obtain sizes of image
	short cxImg = 600,cyImg = 400;
	HRESULT hr;
	if (m_bUseOverlay) // If overlay mode is available, use it
	{
	...
	// 2. Second, initialize dialog and controls
	InitLayout();
	int cxScreen = ::GetSystemMetrics(SM_CXFULLSCREEN);
	int cyScreen = ::GetSystemMetrics(SM_CYFULLSCREEN);
	int nSavedX1 = g_BaumerProfile.GetProfileInt(_T("PreviewDlg"), _T("SavedX1"), -1);
	int nSavedY1 = g_BaumerProfile.GetProfileInt(_T("PreviewDlg"), _T("SavedY1"), -1);
	int nSavedX2 = g_BaumerProfile.GetProfileInt(_T("PreviewDlg"), _T("SavedX2"), -1);
	int nSavedY2 = g_BaumerProfile.GetProfileInt(_T("PreviewDlg"), _T("SavedY2"), -1);
	...
	if (m_bBoardOK)
	{
		if (m_bUseOverlay)
		{
			...
			// If ovelay mode exists, but does not work, use preview.
			if (!m_bOvrMode)
			{
		...
		} // if overlay mode doesn't exists, preview mode already set 
	}
	m_bInited = true;
	if (g_BaumerProfile.GetProfileInt(_T("Settings"), _T("HideHelp"), 0, true))
...
CSize CPreviewDlg::CalcSizesByCxWindow(int cxWindow)
{
	...
	int cyImage = cxRealImg?cxImage*cyRealImg/cxRealImg:cxImage;
	cyImage = max(cyImage, 5*m_szButton.cy+4*m_nGap);
...
CSize CPreviewDlg::CalcSizesByCyWindow(int cyWindow)
{
	...
	int cxImage = 0;
	if( cyRealImg > 0  )
		cxImage = cyRealImg?cyImage*cxRealImg/cyRealImg:cyImage;
...
LRESULT CPreviewDlg::OnUserMsg(WPARAM wParam, LPARAM lParam)
{
	...
	{ // For overlay mode
		if (m_bInited)
		{
			...
			CRect rcWnd;
			GetWindowRect(rcWnd);
			g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedX1"), rcWnd.left);
			g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedY1"), rcWnd.top);
			g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedX2"), rcWnd.left+cx);
			g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedY2"), rcWnd.top+cy);
		}
	}
	else
	{
...
{
	CSettingsSheet dlg(this,m_pPreview?(IUnknown*)m_pPreview:(IUnknown*)m_pOverlay);
...
void CPreviewDlg::OnSize(UINT nType, int cx, int cy) 
{
	...
	if (m_bInited /*&& !s_bOnSetWindowPos*/ )
	{
		...
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedX1"), rcWnd.left);
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedY1"), rcWnd.top);
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedX2"), rcWnd.left+rc.Width());
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedY2"), rcWnd.top+rc.Height());
	}
}
...
void CPreviewDlg::OnMove(int x, int y) 
{
	...
	if (m_bInited && !s_bOnSetWindowPos)
	{
		...
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedX1"), rcWnd.left);
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedY1"), rcWnd.top);
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedX2"), rcWnd.right);
		g_BaumerProfile.WriteProfileInt(_T("PreviewDlg"), _T("SavedY2"), rcWnd.bottom);
	}
}
