...
#include "FilterAphine.h"
#include "misc_utils.h"
#include "nameconsts.h"

//[AY]
_ainfo_base::arg	CFilterNewResizeInfo::s_pargs[] = 
{	
	/*name              type               def value    argument      data object*/
	{"NewWidth",		szArgumentTypeInt, "0",        true,          false },

	{"NewHeight",		szArgumentTypeInt, "0", true, false },
	{"Img",				szArgumentTypeImage, 0, true, true },
	{"Result",			szArgumentTypeImage, 0, false, true },	
	{0, 0, 0, false, false },
};

_ainfo_base::arg	CFilterNewRotateInfo::s_pargs[] = 
{	
	{"Angle",			szArgumentTypeDouble, "0", true, false },
	{"Img",				szArgumentTypeImage, 0, true, true },
	{"Result",			szArgumentTypeImage, 0, false, true },	
	{0, 0, 0, false, false },
};

bool CFilterNewResize::InitAphine()
{
	pArg = GetDataArgument();
	pResult = GetDataResult();
	...
	hn = GetArgDouble( "NewHeight" );
	wn = GetArgDouble( "NewWidth" );

	if( hn < 1 )hn = h;
	if( wn < 1 )wn = w;

	_point	point;
	pArg->GetOffset( &point );

	m_params=
      CAphineParams(
	  CMatrix2x2(wn/w, 0, 0, hn/h ),
				CVector2D(point.x,point.y),
				CVector2D(point.x,point.y)/*new*/);

	return true;
}

bool CFilterNewRotate::InitAphine()
{
	pArg = GetDataArgument();
	pResult = GetDataResult();
	...
	double fAngle = GetArgDouble( "Angle" );
	...
	m_params=
      CAphineParams(
	  CMatrix2x2(::cos( fAngle ), ::sin( fAngle ),
              ...
              _size &new_size, _point &new_offset )
{
  ...
  for( long vv=0;vv<size.cy;vv++)
  {
    ...
    for( long uu = 0; uu < size.cx; uu++, pmask++)
    {
      if( *pmask==255 )
      {
        //if (r.x>x_max) x_max=r.x;
        //if (r.y>y_max) y_max=r.y;
        //if (r.x<x_min) x_min=r.x;
                ...
                bool (*pCallBack)(int, long), long lParam )
{
  ...
  {for( int nPane=0;nPane<npanes;nPane++ )
     if(IsPaneEnable(nPane)) nperc1+=size_new.cy;}  
                ...
                bool (*pCallBack)(int, long), long lParam )
{
  ...
  if(!pCallBack(percent, lParam)) return(false);
	...
	bool bIsPaneEnable0 = IsPaneEnable( 0 );
	bool bIsPaneEnable1 = IsPaneEnable( 1 );
	bool bIsPaneEnable2 = IsPaneEnable( 2 );
...
bool MyCallBack(int nPercent, long lParam)
{
	CLongOperationImpl	*p = (CLongOperationImpl	*)lParam;
	return p->Notify( nPercent );
}

//Bilinear filtering (accurate)
bool CFilterAphine::InvokeFilter()
{
  GetAsyncKeyState(VK_ESCAPE); //очистить состояние Esc
  ...
  pArg->GetOffset( &offset );
  
  _point offset_new;
  _size size_new;
  atCalcNewSizeAndOffsetMasked(pArg, m_params,
                         size_new, offset_new );

  // test for atCalcNewSizeAndOffsetMasked
  //offset_new.x-=8;
  //offset_new.y-=8;
  //size_new.cx+=16;
  //size_new.cy+=16;
  ...
  pResult->CreateImage( size_new.cx, size_new.cy, bstrCC );  // ! New image has a blank mask !
  pResult->SetOffset( offset_new, false );
  pResult->InitRowMasks();

  StartNotification( 101, 1, 1 );

  if(m_fast)
    atExecuteFast( pArg, pResult, m_params, MyCallBack, (long)(CLongOperationImpl*)this);
  else
    atExecute( pArg, pResult, m_params, MyCallBack, (long)(CLongOperationImpl*)this);
