...
_ainfo_base::arg        CActionStripExInfo::s_pargs[] =
{
	{"Img",         szArgumentTypeImage, 0, true, true },
	{0, 0, 0, false, false },
};

CActionStripEx::CActionStripEx(void)
{
	m_nStripSize=0;
	m_nDrawingState=0;
	m_ptrImage=0;
...
bool CActionStripEx::DoLButtonDown( _point point )
{
...
bool CActionStripEx::DoRButtonDown( _point point )
{
	Finalize();
	return true;
}

bool CActionStripEx::DoLButtonDblClick( _point point )
{
	Finalize();
	return true;
}

bool CActionStripEx::DoLButtonUp( _point point )
{
...
bool CActionStripEx::DoMouseMove( _point point )
{
	return __super::DoMouseMove( point );
}

bool CActionStripEx::SetPoint(int x, double y)
{
	double *pStrip;
...
bool CActionStripEx::DoTrack( _point point )
{
	int cx=0,cy=0;
	m_ptrImage->GetSize(&cx, &cy);
...
bool CActionStripEx::DoStartTracking( _point point )
{
	return __super::DoStartTracking(point);

}

bool CActionStripEx::DoFinishTracking( _point point )
{
	Finalize();
	return true;
}

void CActionStripEx::EnsureVisible( _point point )
{
...
LRESULT	CActionStripEx::DoMessage( UINT nMsg, WPARAM wParam, LPARAM lParam )
{
...
bool CActionStripEx::Initialize()
{
...
HRESULT CActionStripEx::Paint( HDC hDC, RECT rectDraw,
...
HRESULT CActionStripEx::GetRect( RECT *prect )
{
	if(prect)
	{
		prect->left=0;
...
IUnknownPtr CActionStripEx::get_document()
{
...
HRESULT CActionStripEx::DoInvoke()
{
