...
bool CNamed2DArrays::SetArraySize(int nMaxFrameNo, int nMaxObjectNo, int nMaxItemSize)
{
	...
	if(nMaxFrameNo<nOldMaxFrameNo) return false; // никогда не уменьшать массив
	if(nMaxObjectNo<nOldMaxObjectNo) return false;
	if(nMaxItemSize<nOldMaxItemSize) return false;
