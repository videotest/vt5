...
void CColourPopup::DrawCell(CDC* pDC, int nIndex)
{
    ...
    if (m_strCustomText.GetLength() && nIndex == CUSTOM_BOX_VALUE)
    {
        ...
        pDC->DrawText(m_strCustomText, TextButtonRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE|DT_END_ELLIPSIS);
    ...
    if (m_strDefaultText.GetLength() && nIndex == DEFAULT_BOX_VALUE)
    {
        ...
        CFont *pOldFont = (CFont*) pDC->SelectObject(&m_Font);
        pDC->SetBkMode(TRANSPARENT);
        pDC->DrawText(m_strDefaultText, TextButtonRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE|DT_END_ELLIPSIS);
        pDC->SelectObject(pOldFont);
