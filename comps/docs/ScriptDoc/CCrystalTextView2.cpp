...
void CCrystalTextView::MoveWordLeft(BOOL bSelect)
{
	...
	{
		nPos --;
		if (isalnum_ru(pszChars[nPos]) || pszChars[nPos] == _T('_'))
		{
			while (nPos > 0 && (isalnum_ru(pszChars[nPos - 1]) || pszChars[nPos - 1] == _T('_')))
				nPos --;
		}
		else
		{
			while (nPos > 0 && ! isalnum_ru(pszChars[nPos - 1])
...
void CCrystalTextView::MoveWordRight(BOOL bSelect)
{
	...
	if (isalnum_ru(pszChars[nPos]) || pszChars[nPos] == _T('_'))
	{
		while (nPos < nLength && isalnum_ru(pszChars[nPos]) || pszChars[nPos] == _T('_'))
			nPos ++;
	}
	else
	{
		while (nPos < nLength && ! isalnum_ru(pszChars[nPos])
...
void CCrystalTextView::MoveHome(BOOL bSelect)
{
	...
	LPCTSTR pszChars = GetLineChars(m_ptCursorPos.y);
	int nHomePos = 0;
	while (nHomePos < nLength && isspace_ru(pszChars[nHomePos]))
		nHomePos ++;
...
CPoint CCrystalTextView::WordToRight(CPoint pt)
{
	...
	while (pt.x < nLength)
	{
		if (! isalnum_ru(pszChars[pt.x]) && pszChars[pt.x] != _T('_'))
			break;
		pt.x ++;
	}
	ASSERT_VALIDTEXTPOS(pt);
...
CPoint CCrystalTextView::WordToLeft(CPoint pt)
{
	...
	while (pt.x > 0)
	{
		if (! isalnum_ru(pszChars[pt.x - 1]) && pszChars[pt.x - 1] != _T('_'))
			break;
		pt.x --;
	}
	ASSERT_VALIDTEXTPOS(pt);
