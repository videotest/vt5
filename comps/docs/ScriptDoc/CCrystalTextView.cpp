...
IMPLEMENT_DYNAMIC(CCrystalTextView, CCmdTargetEx)

HINSTANCE CCrystalTextView::s_hResourceInst = NULL;

BEGIN_MESSAGE_MAP(CCrystalTextView, CWnd)
...
//	CWnd::OnInitialUpdate();
	...
	CSplitterWnd *pSplitter = 0;//GetParentSplitter(this, FALSE);
	if (pSplitter != NULL)
	{
...
void CCrystalTextView::RecalcVertScrollBar(BOOL bPositionOnly /*= FALSE*/)
{
	...
	else
	{
		...
		si.nMin = 0;
		si.nMax = max( GetLineCount() - 1, 0 );
		si.nPage = max( GetScreenLines(), 0 );
		si.nPos = m_nTopLine;
	}
...
void CCrystalTextView::RecalcHorzScrollBar(BOOL bPositionOnly /*= FALSE*/)
{
	...
	else
	{
		...
		si.fMask = SIF_DISABLENOSCROLL | SIF_PAGE | SIF_POS | SIF_RANGE;
		si.nMin = 0;
		si.nMax = max( GetMaxLineLength() - 1, 1 );
		si.nPage = max( GetScreenChars(), 0 );
		si.nPos = m_nOffsetChar;
	}
