...
#include "\vt5\crypto\CryptedFile.h"
...
IMPLEMENT_DYNCREATE(CCrystalTextBuffer, CCmdTargetEx)
...
BEGIN_MESSAGE_MAP(CCrystalTextBuffer, CCmdTargetEx)
...
BOOL CCrystalTextBuffer::LoadFromFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/)
{
	...
	do
	{
		...
		//TODO: для protected-файлов - сюда CheckFileSignature(pszFileName, false)
		bool bSigned = CheckFileSignature(pszFileName);
		...
		// TODO: для protected-файлов - сюда CSimpleCrypter::Decrypt(...)
		if(bSigned)
			sc.DecryptBuf(pcBuf, dwCurSize);
		...
		while (dwBufPtr < dwCurSize)
		{
			...
			if (dwBufPtr == dwCurSize && dwCurSize == dwBufSize)
			{
				...
				// TODO: для protected-файлов - сюда CSimpleCrypter::Decrypt(...)
				if(bSigned) sc.DecryptBuf(pcBuf, dwCurSize);
				dwBufPtr = 0;
			}
		...
		if(bSigned)
		{ // подписанные файлы прогоняем через обфускатор
			IUnknownPtr	ptrAM = 0;
			...
			for (int nLine = 0; nLine < nLineCount; nLine ++)
			{
				...
				CString str(pli->m_pcLine, pli->m_nLength);
				int i=0;
				while(1)
				{
					i = str.Find("ActionManager.",i);
					if(i<0) break;
					i += 14; // len("ActionManager.")
					int j=i;
					while(isalnum_ru(str[j]) || '_'==str[j]) j++;
					if(j==i) continue;

					BSTR bstr = str.Mid(i, j-i).AllocSysString();
					sptrApp2->ObfuscateName(&bstr);
					str.Delete(i, j-i);
					str.Insert(i, CString(bstr));
					SysFreeString(bstr);
				}

				i=0;
				while(1)
				{ // найдем все "StealthBit"
					i = str.Find("StealthBit");
					if(i<0) break;
					str.Delete(i,10); // len("StealthBit") = 10

					int j=i;
					while(isdigit_ru(str[j])) j++;
					if(j==i) continue;

					int nIndex = atoi(str.Mid(i, j-i));
					bool bValue = (!GetStealthKeysNumber()) || StealthIndexIsAvailable( nIndex );
					// либо ключа нет (демоверсия), либо есть галочка
					CString s;
					s.Format( "%i", bValue ? 1 : 0 );
				...
				pli->m_nLength = nLength;
				pli->m_nMax = ALIGN_BUF_SIZE(pli->m_nLength);
				ASSERT(pli->m_nMax >= pli->m_nLength);
				if (pli->m_nMax > 0)
					pli->m_pcLine = new TCHAR[pli->m_nMax];
				if (pli->m_nLength > 0)
					memcpy(pli->m_pcLine, str.GetBuffer(), sizeof(TCHAR) * pli->m_nLength);
...
BOOL CCrystalTextBuffer::InternalInsertText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar)
{
	...
	for (;;)
	{
		bool	bChangeTextPos = false;
		nTextPos = 0;
		while (pszText[nTextPos] != 0 && pszText[nTextPos] != _T('\n'))
			nTextPos ++;

		if( nTextPos > 0 && pszText[nTextPos-1] == _T('\r') )
		{
			bChangeTextPos = true;
			nTextPos--;
		}
...
void CCrystalTextBuffer::RemoveLine(int nLine)
{
	if(nLine < m_aLines.GetSize())
	{
		delete m_aLines[nLine].m_pcLine;
