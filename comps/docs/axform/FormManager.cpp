...
long CFormManager::ExecutePage(LPCTSTR szFormName) 
{
	...
	// --- поддержка методик: хитра€ настройка в shell.data, запрещающа€ показ форм
	if( ::GetValueInt( ::GetAppUnknown(), "\\FormManager", "DisableFormShow", 0 ) != 0 )
	{
		::ExecuteScript( _bstr_t("ProcessMessage 0") ); // [SBT 1163]
		PressOK(); // если не показываем форму - сразу нажмем в ней Ok, пусть подавитс€ хом€к
		::ExecuteScript( _bstr_t("ProcessMessage 0") );	// [SBT 1163]

		// 27.09.2005 build 91 SBT 1440
		// если помимо формы была запущена интерактивна€ акци€ - ее тоже прервем
		// не помогло (т.к. акци€ запускаетс€ уже после смерти формы), но пусть будет.
...
{
	CTempValue<bool> tmp(&m_bModalMode, true); // на врем€ выполнени€ поставить m_bModalMode = true;
...
void CFormManager::PressCancel() 
{
	...
	{ // даже если кнопки задизаблены - Cancel будем обрабатывать (откладыва€ на потом)
		if(m_pFormPage)
			m_pFormPage->m_site.m_bCancelSheduled = true;
		return;
	}
