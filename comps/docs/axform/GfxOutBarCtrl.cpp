...
CGfxOutBarCtrl::CGfxOutBarCtrl()
{
	...
	if (crTextColor == crBackGroundColor)
		crTextColor = RGB(~GetRValue(crTextColor),~GetGValue(crTextColor),~GetBValue(crTextColor));
...
bool CGfxOutBarCtrl::UseAdvMode(bool bUse)
{
	bool bPrevSetting = m_bUseAdvMode;
	m_bUseAdvMode = bUse;
	return bPrevSetting;
}
...
{
	ASSERT(!(!m_bUseAdvMode && arFolder.GetSize() > 0 ));
...
void CGfxOutBarCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	_ID_GFX_SMALLICON.LoadString( ID_GFX_SMALLICON );
	_ID_GFX_LARGEICON.LoadString( ID_GFX_LARGEICON );
	_ID_GFX_REMOVEITEM.LoadString( ID_GFX_REMOVEITEM );
	_ID_GFX_RENAMEITEM.LoadString( ID_GFX_RENAMEITEM );
...
void CGfxOutBarCtrl::SetActiveItem(int nItem)
{
	CRect	rc;
	if( iLastSel != -1 )
	{
