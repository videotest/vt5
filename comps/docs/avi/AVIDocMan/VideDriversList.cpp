...
//			if (sCat != _T("Input")) continue;
		...
		if (m_saLongNames.GetSize() == 1 && CStdProfileManager::m_pMgr->GetProfileInt(NULL,
			_T("SkipSelectDriverWhen1Driver"), TRUE, true))
		{
			CString s = m_saShortNames.GetAt(0);
			CStdProfileManager::m_pMgr->WriteProfileString(NULL, _T("VideoDrvName"), s);
			EndDialog(IDOK);
			return FALSE;
		}
