		...
		_bstr_t sSection(_T("\\Database\\Blank\\"));
		{
			...
			if (bstrName.length() > 0)
			{
				sSection += (LPCTSTR)bstrName;
				{
					...
					if(m_ViewSubType>0){
						if(4==m_ViewSubType){
							nZoomingModeDefault=dcmTopLeft;
						}else{
							nZoomingModeDefault=dcmAlreadyZoom;
						}
						_bstr_t bsSubView((long)m_ViewSubType);
						sSection += L"\\"+bsSubView;
					}
					nZoomingMode= ::GetValueInt(::GetAppUnknown(), sSection, "ZoomingMode", nZoomingModeDefault);
				}
			}
