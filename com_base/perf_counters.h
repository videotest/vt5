...
static		TCHAR       INDEX_PROCTHRD_OBJ[255] = TEXT("");
		...
		if( pprocess_object )
		{
			...
			if( pinst_find )
			{
...

				{
					PPERF_COUNTER   pcounter = 0;
					DWORD*			pdw_data = 0;
					pcounter = FindCounter( pprocess_object, PX_PROCESS_ID );
					if( pcounter )
					{
						pdw_data = (DWORD *) CounterData( pinst_find, pcounter );
						if( pdw_data )						
...
protected:
	...
	void		deinit();
public:
	//at first need open process
	bool		open_process( const char* psz_process );
	//VM size
	DWORD		get_vm_size();
	//GDI obkect count
	DWORD		get_gdi_count();
protected:
	//global data
	DWORD           m_dw_perf_size;
	PPERF_DATA      m_pperf_data;
	PPERF_OBJECT    m_pprocess_object;
	
	//process data
	PPERF_INSTANCE  m_pinst_process;
	PPERF_COUNTER   m_pcounter_vm;
	DWORD			m_dw_process_id;
	HANDLE			m_hprocess;

	//resource count poiner
	GETGUIRESOURCES_PROC m_pfnGetGuiResources;
};

////////////////////////////////////////////////////////////////////////////
inline task_info::task_info()
{
	m_pfnGetGuiResources = 0;
...
////////////////////////////////////////////////////////////////////////////
inline void task_info::init_def()
{
	m_dw_perf_size		= 0;
	m_pperf_data		= 0;
	m_pprocess_object	= 0;
...
////////////////////////////////////////////////////////////////////////////
inline bool task_info::open_pref_data()
{
	SetPerfIndexes( 0 );

	deinit();
	
	m_dw_perf_size = 50*1024;
    m_pperf_data = RefreshPerfData( ghPerfKey, INDEX_PROCTHRD_OBJ, m_pperf_data, &m_dw_perf_size );
	if( !m_pperf_data )			return false;

	m_pprocess_object = FindObject( m_pperf_data, PX_PROCESS );
	if( !m_pprocess_object )	return false;

	return true;
}

////////////////////////////////////////////////////////////////////////////
inline void task_info::deinit()
{
	if( m_pperf_data )
		::LocalFree( m_pperf_data );	m_pperf_data = 0;
	
	if( m_hprocess )
		::CloseHandle( m_hprocess );	m_hprocess = 0;
...
//at first need open process
////////////////////////////////////////////////////////////////////////////
inline bool task_info::open_process( const char* psz_process )
{
	if( !m_pprocess_object )
		open_pref_data();
	...
	pinst = FirstInstance ( m_pprocess_object );
	while( pinst && instance_idx < m_pprocess_object->NumInstances )
	{
		BSTR bstr = InstanceName( pinst );
		if( bstr )
		{
	...
	if( !m_pinst_process )	return false;
	
	//open counters
	m_pcounter_vm = FindCounter( m_pprocess_object, PX_PROCESS_PRIVATE_PAGE );

	//get process id
	{
		PPERF_COUNTER   pcounter = 0;
		DWORD*			pdw_data = 0;
		pcounter = FindCounter( m_pprocess_object, PX_PROCESS_ID );
		if( pcounter )
		{
			pdw_data = (DWORD*) CounterData( m_pinst_process, pcounter );
			if( pdw_data )						
				m_dw_process_id = *pdw_data;
		}
	}

	if( m_dw_process_id )
		m_hprocess = ::OpenProcess( PROCESS_QUERY_INFORMATION|PROCESS_VM_READ, FALSE, m_dw_process_id );
...
//VM size
////////////////////////////////////////////////////////////////////////////
inline DWORD task_info::get_vm_size()
{
	if( !m_pinst_process || !m_pcounter_vm )	return 0;

	DWORD dw_size = 0;

	{
		DWORD* pdw_data = 0;
		pdw_data = (DWORD*)CounterData( m_pinst_process, m_pcounter_vm );
		if( pdw_data )						
			dw_size = *pdw_data;
	}
...
//GDI object count
////////////////////////////////////////////////////////////////////////////
inline DWORD task_info::get_gdi_count()
{
	...
	return m_pfnGetGuiResources( m_hprocess, GR_GDIOBJECTS);
}