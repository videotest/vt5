...
class CCellObj : public CObject
{
	...
	CBCGColorPickerCtrl *m_pParent;
	CCellObj (	CPalette* pPalette,
				const COLORREF color, 
				const int x, const int y, 
				const int nCellWidth,
				const long lDelta,
				CBCGColorPickerCtrl *pParent)
	{
		m_x = x;
		m_y = y;
		...
		if (globalData.m_nBitsPerPixel == 8) // 256 colors
		{
			...
			m_CellColor = PALETTEINDEX (uiPalIndex);
		}
		else
		{
			//-----------------------------------------------
			// Approximate color to onr of "standard" colors:
			//-----------------------------------------------
	...
	void Draw (CDC* pDC)
	{
		...
		COLORREF CellColor = m_CellColor;
		if (m_pParent->m_bPalette)
			CellColor = m_pParent->m_Palette[GetGValue(m_CellColor)];
		CBrush br (CellColor);
		CPen pen (PS_SOLID, 1, CellColor);
...
CBCGColorPickerCtrl::CBCGColorPickerCtrl()
{
	...
	RGBtoHSL (m_colorNew, &m_dblHue, &m_dblSat, &m_dblLum);
...
void CBCGColorPickerCtrl::SetDrawPalette(COLORREF *pColor)
{
	m_bPalette = pColor != 0;
	if (pColor)
	...
	for (int nRowNum = 0; nRowNum < 2; nRowNum++)
	{
		if (nRowNum == 1)
		{
			...
			m_arCells.Add (new CCellObj (
				m_pPalette, colorWhite, x1, yCenter, nCellLargeSize, 0, this));
		}
		...
		for (int i = 0; i < GRAY_CELLS_NUM; i++)
		{
			...
			if (nRowNum == 1)
			{
				m_arCells.Add (new CCellObj (m_pPalette, color, x, nCurry, nCellSize, 7, this));
			}
		...
		if (nRowNum == 1)
		{
			...
			m_arCells.Add (new CCellObj (
				m_pPalette, colorBlack, x1, yCenter, nCellLargeSize, 0, this));
		}
...
void CBCGColorPickerCtrl::CreateHexagon ()
{
	...
	// Add center cell
	m_arCells.Add (new CCellObj (m_pPalette, colorWhite, x, y, nCellSize, 0, this));
		...
		for (int nSide = 0; nSide < NUM_LEVELS - 1; nSide++)
		{
			...
			for (int nCell = 0; nCell < nLevel; nCell++)
			{
				...
				m_arCells.Add (new CCellObj (m_pPalette, 
					HLStoRGB_TWO ((float) nAngle, L, 1.0F), 
					nPosX, nPosY, nCellSize, 16, this));
			...
			m_bmpPicker.CreateCompatibleBitmap (pDC, szColorPicker.cx, szColorPicker.cy))
		{
			...
			for (int i= 0;i<szColorPicker.cy;i += nStep)
			{
				for(int j=0;j<szColorPicker.cx;j += nStep)
				{
					...
					COLORREF color = HLStoRGB_ONE((double)j/(double)szColorPicker.cx, DEFAULT_LUMINANCE, (double)i/(double)szColorPicker.cy);
...
void CBCGColorPickerCtrl :: DrawLuminanceBar(CDC* pDC)
{
	...
	{
		COLORREF col = 	m_bPalette?m_Palette[max(min(int(LumFromPoint(y)*255),255),0)]:
			HLStoRGB_ONE (m_dblHue, LumFromPoint (y), m_dblSat);
...
void CBCGColorPickerCtrl::SetRGB (COLORREF ref)
{
	...
	RGBtoHSL (ref, &m_dblHue, &m_dblSat, &m_dblLum);
	m_colorNew = ref;
...
void CBCGColorPickerCtrl::SetHLS(double hue, double luminance, double saturation, BOOL bInvalidate)
{
	...
	m_colorNew = HLStoRGB_TWO(m_dblHue, m_dblSat, m_dblLum);
	...
	case LUMINANCE:
		{
			...
			m_colorNew = HLStoRGB_ONE(m_dblHue, m_dblLum, m_dblSat);
...
void CBCGColorPickerCtrl::DrawItem (LPDRAWITEMSTRUCT lpDIS)
{
	...
	case CURRENT:
		{
			...
			COLORREF colorNew = m_colorNew;
			COLORREF colorOriginal = m_colorOriginal;
			if (m_bPalette)
			{
				colorNew = m_Palette[GetGValue(colorNew)];
				colorOriginal = m_Palette[GetGValue(colorOriginal)];
			}

			int nHalf = rectClient.Height () / 2;
			pDC->FillSolidRect (0, 0, rectClient.Width (), nHalf, colorNew);
			pDC->FillSolidRect(0, nHalf, rectClient.Width (), nHalf, colorOriginal);
	...
	case LUMINANCE:
		...
		// Draw marker:
		pDC->FillRect (CRect (m_nLumBarWidth, 0, 
