...
void CColorPage2::OnHLSChanged() 
{
	...
	COLORREF color = HLStoRGB_ONE (dblH, dblL, dblS);
...
void CColorPage2::Setup (BYTE R, BYTE G, BYTE B)
{
	...
	double luminance;
	double saturation;
	RGBtoHSL((COLORREF)RGB(R, G, B), &hue, &saturation, &luminance);
...
void CColorPage2::OnLuminance() 
{
	...
	COLORREF color = HLStoRGB_ONE(H, L, S);
...
void CColorPage2::OnColour() 
{
	...
	// Set actual color.
	m_pDialog->SetNewColor(HLStoRGB_ONE(H, L, S));
	m_pDialog->SetPageOne((BYTE) m_r, (BYTE) m_g, (BYTE) m_b);
