...
void CButtonAppearanceDlg::OnAddImage() 
{
	...
	try
	{
		...
		CBCGImageEditDlg dlg (&bitmap, this);
		if (dlg.DoModal () != IDOK)
		{
			return;
		}
...
void CButtonAppearanceDlg::OnEditImage() 
{
	...
	try
	{
		...
		CBCGDrawState ds;
		COLORREF cl = m_pImages->GetTransparentColor();
		m_pImages->SetTransparentColor( -1 );
		...
		m_pImages->SetTransparentColor( cl );
...
BOOL CButtonAppearanceDlg::OnInitDialog() 
{
	...
	if (pImages != NULL)
	{
		m_rectDefaultImage.right = 
			m_rectDefaultImage.left + pImages->GetImageSize ().cx;
		m_rectDefaultImage.bottom = 
			m_rectDefaultImage.top + pImages->GetImageSize ().cy;
	}
...
{
	bool	bImage = (m_bImage||m_bMenuMode);
	m_wndButtonText.EnableWindow (m_bText && (!m_bMenuMode || m_pUserTool == NULL));

	m_wndButtonList.EnableWindow (bImage && m_bUserButton /*&& !m_bMenuMode||m_bMenuMode*/);
	m_wndAddImage.EnableWindow (bImage && m_pImages != NULL && m_bUserButton && 
		/*!m_bMenuMode &&*/ !m_pImages->IsReadOnly ());
	m_wndEditImage.EnableWindow (bImage && m_pImages != NULL && m_iSelImage >= 0 && 
		m_bUserButton && !m_pImages->IsReadOnly ());

	m_wndUserImageBtn.EnableWindow (bImage && m_pImages != NULL /*&& !m_bMenuMode*/);

	m_wndDefautImageBtn.EnableWindow (m_pUserTool != NULL ||
		( bImage && CBCGToolBar::GetDefaultImage (m_pButton->m_nID) >= 0) /*&&
		!m_bMenuMode*/);
