...
void CSizingControlBar::StartTracking(UINT nHitTest)
{
    ...
    else
    {
		...
		int	nPosition = i +((m_htEdge == HTTOP || m_htEdge == HTLEFT) ? -1 : 1);
		if( nPosition <= -1 || nPosition >= arrSCBars.GetSize() )
			m_szMaxT = m_szOld;
		else
		{
			CSizingControlBar* pBar = arrSCBars[nPosition];
...
void CSizingControlBar::OnTrackUpdateSize(CPoint& point)
{
    ...
    for (int i = 0; i < arrSCBars.GetSize(); i++)
        ...
        {   // adjust the neighbour's size too
			...
			int	nNo =i + ((m_htEdge == HTTOP || m_htEdge == HTLEFT) ? -1 : 1);

			if( nNo > 0 )
			{
				CSizingControlBar* pBar = arrSCBars[ nNo ];
