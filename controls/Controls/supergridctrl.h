...
control_export_class CItemInfo //should have called this LV_INSERTITEM ..what ever
{
public:
	...
	COLORREF GetSubitemColor(int iSubItem) const
	{
		COLORREF clref;
	...
	{
		int iImage;
		if (!m_mapImages.Lookup(iSubItem,iImage))
			return -1;
		else
			return iImage;
	}
	...
	{
		m_mapImages.SetAt(iSubItem, iImage);
	}
	...
	void CopyImages(CItemInfo* pItemInfo)
	{
		for(int nCol=0; nCol < pItemInfo->GetItemCount(); nCol++)
		{
			int nImage;
			if(pItemInfo->m_mapImages.Lookup(nCol, nImage))
				m_mapImages.SetAt(nCol, nImage);
		}
	}
...
interface ISuperGridSite
{
	virtual void OnCheckStateChanged(CItemInfo *pInfo) = 0;
	virtual void OnSelectionChanged(CItemInfo *pInfo) = 0;
};

#define LVIR_EX_TREE               4
#define LVIR_EX_TREE_SIGN          5
#define LVIR_EX_TREE_LINE          6

#define LVS_EX_EXPAND_CHECK        1
...
//
/////////////////////////////////////////////////////////////////////////////
control_export_class CSuperGridCtrl : public CListCtrl
{
// Construction
public:
	CSuperGridCtrl();
...
public:
	...
	void SetTreeSubItem(int nTreeSubItem);
	int	 GetTreeSubItem() const {return m_nTreeSubItem;};
	...
	// Functyons for works with flags
	DWORD SetFlags(DWORD dwFlags);
	DWORD GetFlags() const {return m_dwFlags;}
	void SetSite(ISuperGridSite *pSite) {m_pSite = pSite;}
...
protected:
	...
	BOOL GetSubItemRect(int nItem, int nSubItem, int nRectType, CRect &rc);
private:
	BOOL GetMultyRects(int nItem, int nSubItem, CRect *pRects);
	enum DoSetCheckFlags
	{
		DoSetCheck_HIDDEN_ITEMS = 1,
		DoSetCheck_SKIP_THIS = 2,
		DoSetCheck_RECURSIVE = 4,
	};
	void DoSetCheck(CTreeItem* pItem, int iItem, BOOL bCheck, DWORD dFlags);
	//list of rootItems
	...
	HIMAGELIST m_himl;
	int m_nTreeSubItem;
	DWORD m_dwFlags; // const LVS_EX...
	ISuperGridSite *m_pSite;
...
control_export_class CSuperGridCtrlEx : public CSuperGridCtrl
{
	int m_nBmpId;
public:
	...
	CSuperGridCtrlEx();
	void SetBmpId(int id) { m_nBmpId = id;}
	bool CreateImageList(int id, int cx = 16, int nGrow = 1, COLORREF clrBack = RGB(0, 255, 255));
...
control_export_class CRectangle
{
public:
...
//the nested class 
control_export_class CSuperGridCtrl::CTreeItem : public CObject
{
